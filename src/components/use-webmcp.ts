...
export function useWebMCP(handlers: Handlers) {
  ...
  useEffect(() => {
    ...
    const tools: WebMCPTool[] = [
      ...
      {
        name: "reorder_clips", description: "Replace timeline clip order atomically. Include every current clip ID exactly once.",
      ...
      {
        name: "adjust_clip", description: "Adjust a clip's color, volume, mute state, speed or edge fades. Omitted properties remain unchanged.",
        inputSchema: mutationSchema({ clip_id: string("Clip ID"), brightness: { type: "number", minimum: -1, maximum: 1 }, contrast: { type: "number", minimum: 0, maximum: 2 }, saturation: { type: "number", minimum: 0, maximum: 3 }, hue: { type: "number", minimum: -180, maximum: 180 }, volume: { type: "number", minimum: 0, maximum: 2 }, muted: { type: "boolean" }, speed: { type: "number", minimum: 0.5, maximum: 2 }, fade_in_ms: number("Fade-in duration"), fade_out_ms: number("Fade-out duration") }, ["clip_id"]),
        async execute(input) { assertVersion(input); const patch: Record<string, number | boolean> = {}; for (const [source, target] of [["brightness", "brightness"], ["contrast", "contrast"], ["saturation", "saturation"], ["hue", "hue"], ["volume", "volume"], ["speed", "speed"], ["fade_in_ms", "fadeInMs"], ["fade_out_ms", "fadeOutMs"]] as const) if (input[source] !== undefined) patch[target] = asNumber(input[source], source); if (input.muted !== undefined) { if (typeof input.muted !== "boolean") throw new Error("muted must be a boolean"); patch.muted = input.muted; } return current.current.dispatch({ type: "adjust_clip", actor: "agent", clipId: asString(input.clip_id, "clip_id"), patch }); },
      },
      {
